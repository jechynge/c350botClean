#include "Common.h"
#include "ReaverManager.h"

ReaverManager::ReaverManager() { }

// targets vector will only contain valid targets
void ReaverManager::executeMicro(const UnitVector & targets)
{
	...
	{
		// if this reaver is loaded into a shuttle, keep going.
		...
		if (order.type == order.Attack || order.type == order.Defend) {
			...
			{
				// find the best target for this reaver
		...
		// If we're harassing worker lines, the shuttles are moving us around.
		else if (order.type == order.HarassWorkers)
		{
			BWAPI::Unit* target = getTarget(rangedUnit, rangedUnitTargets);
...
void ReaverManager::kiteTarget(BWAPI::Unit * rangedUnit, BWAPI::Unit * target)
{
...
BWAPI::Unit * ReaverManager::getTarget(BWAPI::Unit * rangedUnit, UnitVector & targets)
{
...
int ReaverManager::getAttackPriority(BWAPI::Unit * rangedUnit, BWAPI::Unit * target)
{
...
BWAPI::Unit * ReaverManager::closestrangedUnit(BWAPI::Unit * target, std::set<BWAPI::Unit *> & rangedUnitsToAssign)
{
